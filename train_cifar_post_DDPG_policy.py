...
class Backdoor_Env(gym.Env):
    ...
    def step(self, action):
        ...
        action[0] = min(int(action[0]*5.5+5.5),10) #index of poison frac [0:1:10]
        ...
        for cid in common(self.cids,att_ids):
            ...
            new_weight=get_parameters(self.net)
            #loss, _= test(self.net2, val_loader)
            #if np.isnan(loss): new_weight=self.aggregate_weights
            #poi_loss, _= test(self.net2, poisoned_val_loader)
        ...
        #for cid in common(self.cids,att_ids):
            #new_weight=craft(self.aggregate_weights, average(att_weights_lis), -1, action[3])
            #new_weights.append(new_weight)
        
        # Compute average weights
        self.aggregate_weights =  average(new_weights,num_clients, subsample_rate)
        ...
        if len(common(self.cids, att_ids))>=1:
            #set_parameters(self.net,self.aggregate_weights)
            set_parameters(self.net2, self.aggregate_weights)

            #new_loss, new_acc = test(self.net, val_loader)
            #poi_loss, poi_acc= test(self.net, poisoned_val_loader)
            _, clean_acc = test(self.net2, val_loader)
            _, clean_poi_acc= test(self.net2, poisoned_val_loader)
            #print('post_poi_acc:', clean_poi_acc)
            ...
            #reward=clean_poi_acc-self.clean_poi_acc
            reward=clean_acc+clean_poi_acc
            #self.loss=new_loss
            #self.acc=new_acc
            #self.poi_acc=poi_acc
            #self.clean_poi_acc=clean_poi_acc
        ...
        return len(common(self.cids, att_ids)), reward, done, {}
    ...
    def reset(self):
        ...
        #_, clean_acc = test(self.net2, val_loader)
        #self.clean_loss, self.clean_poi_acc= test(self.net2, poisoned_val_loader)
        #print('post_poi_acc:', self.clean_poi_acc)
        ...
        return len(common(self.cids, att_ids))
...
model = DDPG("MlpPolicy", env, action_noise=action_noise, verbose=1, optimize_memory_usage=False, buffer_size=100000, gamma=0.99, 
             learning_rate=1e-3, tau=0.005, learning_starts=100, train_freq=(1,'episode'), batch_size=100)

#model.save("plr1e-3_whitebox_reward_epoch500_cifar_post_100c_10a_TD3_"+str(0))

for i in range(1,101):
    model.learn(total_timesteps=500, log_interval=1, reset_num_timesteps=False) 
    model.save("plr1e-3_whitebox_reward_epoch500_cifar_post_100c_10a_DDPG_"+str(i*500))
